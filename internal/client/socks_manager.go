...
func (c *Client) handleSocksUDPAssociate(ctx context.Context, conn net.Conn, clientAddr string, clientPort uint16, atyp byte) {
	...
	for {
		...
		// Echo the original ATYP/ADDR/PORT (the DNS server the client queried)
		// back in the SOCKS5 UDP reply header. RFC 1928 wants the reply to carry
		// the source address of the datagram; hardcoding 0.0.0.0:53 makes strict
		// clients (e.g. tun2socks-based VPNs) drop the reply because its source
		// address does not match the address they sent the query to.
		// buf[3:payloadOffset] is ATYP+ADDR+PORT of the request; copy it since
		// the shared read buffer may be overwritten before the async callback runs.
		respHeader := append([]byte{0x00, 0x00, 0x00}, buf[3:payloadOffset]...)

		// Direct-DNS mode: resolve straight through the configured upstream
		// (e.g. Yandex DNS over a VPN-protected socket) instead of tunneling.
		if c.directDNSResolver != nil {
			// Copy the query since buf is reused across iterations.
			q := append([]byte(nil), dnsQuery...)
			if resp, ok := c.directDNSResolver(q); ok && len(resp) > 0 {
				fullResp := append(append([]byte(nil), respHeader...), resp...)
				_, _ = udpConn.WriteToUDP(fullResp, peerAddr)
				continue
			}
			// Fall through to the tunnel path on failure.
		}
		...
		if !isHit {
			// Cache miss: the query was fired into the tunnel and will populate
			// the cache asynchronously (fire-and-forget). Keep the UDP
			// association OPEN so the resolver's retry of the same question
			// arrives on the same session and hits the now-populated cache.
			// Closing here (the previous behaviour) tore down the tun2socks UDP
			// session, so retries never reached us and resolution always timed
			// out. The read deadline above eventually reclaims idle sessions.
			c.log.Debugf("🧳 <yellow>SOCKS5 DNS Miss/Pending - keeping association open for retry.</yellow>")
			continue
		}
	}
}
