...
type Client struct {
	...
	// Optional hook: when set, DNS queries arriving via the local SOCKS5 UDP
	// associate are resolved by this function (e.g. directly through Yandex DNS
	// over a VPN-protected socket) instead of being tunneled. Returns the raw
	// DNS response and true on success; false falls back to the DNS tunnel.
...
// SetDirectDNSResolver installs an optional resolver used for DNS queries that
// arrive through the local SOCKS5 UDP associate. When set, queries are resolved
// by resolve() instead of being sent through the DNS tunnel. Call before Run().
func (c *Client) SetDirectDNSResolver(resolve func(query []byte) ([]byte, bool)) {
